...
export default function SEO({title=DEFAULT_TITLE,description=DEFAULT_DESCRIPTION,image=DEFAULT_IMAGE,url=typeof window!=='undefined'?window.location.href:SITE_URL,noIndex=false,additionalStructuredData=null}) {
  ...
  const schemaOrg={
    ...
    '@graph':[
      ...
      },
      {
        ...
        name:'HYPEPAD',
        url:SITE_URL,
        logo:{'@type':'ImageObject','url':`${SITE_URL}/rocket-logo.svg`},
  ...
  if(additionalStructuredData){
    if(Array.isArray(additionalStructuredData)) schemaOrg['@graph'].push(...additionalStructuredData);
    else schemaOrg['@graph'].push(additionalStructuredData);
  }
  return (
    <>
