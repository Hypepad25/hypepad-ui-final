    ...
    <header className="w-full bg-[#111827] text-white flex items-center justify-between px-4 md:px-8 py-3 relative z-30">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="md:hidden relative">
          ...
          >
            <span
              className={\`block w-6 h-0.5 bg-white transition-transform \${open ? 'rotate-45 translate-y-1.5' : ''}\`}
            />
            <span
              className={\`block w-6 h-0.5 bg-white transition-opacity \${open ? 'opacity-0' : 'opacity-100'}\`}
            />
            <span
              className={\`block w-6 h-0.5 bg-white transition-transform \${open ? '-rotate-45 -translate-y-1.5' : ''}\`}
            />
            ...
            <div className="fixed inset-0 z-40 flex">
              ...
              <div className="w-72 bg-[#1f2937] p-6 flex flex-col gap-5 overflow-auto">
                ...
                <nav className="flex flex-col gap-3 text-sm">
                  {LINKS.map(([label, href]) => (
                    <a key={href} href={href} className="hover:underline" onClick={() => setOpen(false)}>
                      {label}
                    </a>
                  ))}
